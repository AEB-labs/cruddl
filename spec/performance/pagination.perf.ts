...
import { initEnvironment, TestEnvironment } from './support/helpers.js';

export async function addManyPapersWithAQL(environment: TestEnvironment, count: number) {
    await environment
        .getDB()
        .query(aql`FOR i IN 1..${count} INSERT { title: CONCAT("Test ", RAND()) } IN papers`);
}
...
function testPagination(config: {
    ...
    return {
        name:
            `Fetch ${config.pages} pages of size ${config.pageSize} from ${config.rootEntitiesInDB} root entities in total` +
        ...
        async fn() {
            ...
            for (let i = 0; i < config.pages; i++) {
                const result: any = await env.exec(
                    `query($count: Int!, $cursor: String, $orderBy: [PaperOrderBy!]) { allPapers(first: $count, after: $cursor, orderBy: $orderBy) { title, id, _cursor } }`,
                    {
