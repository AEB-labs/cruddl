...
describe('Billing validation', () => {
    it('accepts valid definition', () => {
        ...
        const parsedProject = parseProject(new Project([new Source(graphql, 'graphql.graphql'), new Source(JSON.stringify(validDef), 'billing.json')]), new ValidationContext());
        ...
        const parsedProject2 = parseProject(new Project([new Source(graphql, 'graphql.graphql'), new Source(JSON.stringify(validDef2), 'billing.json')]), new ValidationContext());
    ...
    it('rejects typeNames that do not exist', () => {
        ...
        expect(validationContext.asResult().getErrors()[0].message).to.contain('No rootEntity with the name "MissionWithTypo" is defined.');
    });
    ...
    it('rejects keyFieldNames that do not exist', () => {
        ...
        expect(validationContext.asResult().hasWarnings()).is.false;
        expect(validationContext.asResult().getErrors().length).to.eq(1);
        expect(validationContext.asResult().getErrors()[0].message).to.contain('The field "titleWithTypo" is not defined in the type "Mission".');
    });
