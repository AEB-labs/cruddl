...
describe('add-orderby-arguments-to-fields', () => {
    it('meets preconditions', () => {
        ...
        expect(orderByArg).to.be.undefined;
    });
    ...
    it ('adds a new arg `order by` to Query.allFoos() of type FooOrderBy', () => {
        ...
        const allFoosField = queryType.fields.find(field => field.name.value === 'allFoos');
        const orderByArg = allFoosField!.arguments.find(arg => arg.name.value === ORDER_BY_ARG);
        expect(orderByArg).to.not.be.undefined;
        expect(orderByArg!.type.kind).to.equal(LIST_TYPE);
        expect((orderByArg!.type as ListTypeNode).type.kind).to.equal(NON_NULL_TYPE);
        expect((((orderByArg!.type as ListTypeNode).type) as NonNullTypeNode).type.kind).to.equal(NAMED_TYPE);
        expect(((((orderByArg!.type as ListTypeNode).type) as NonNullTypeNode).type as NamedTypeNode).name.value).to.equal('FooOrderBy');
    });
