...
describe('Decimal2', () => {
    describe('parseLiteral', () => {
        it('accepts positive ints', () => {
            expect(GraphQLDecimal2.parseLiteral({ kind: 'IntValue', value: '123' }, {})).to.equal(
                123,
            );
        });
        ...
        it('accepts negative ints', () => {
            expect(
                GraphQLDecimal2.parseLiteral({ kind: 'IntValue', value: '-1000000000' }, {}),
            ).to.equal(-1000000000);
        });

        it('accepts numbers with one decimal digit', () => {
            expect(
                GraphQLDecimal2.parseLiteral({ kind: 'FloatValue', value: '123.4' }, {}),
            ).to.equal(123.4);
        });

        it('accepts numbers with two decimal digits', () => {
            expect(
                GraphQLDecimal2.parseLiteral({ kind: 'FloatValue', value: '123.45' }, {}),
            ).to.equal(123.45);
        });
