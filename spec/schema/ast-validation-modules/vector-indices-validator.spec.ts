...
describe('vector indices validator', () => {
    ...
    it('rejects vector index on non-numeric list field', () => {
        assertValidatorRejects(
            `
            type Foo @rootEntity {
        ...
        `,
            'Vector indices can only be defined on list fields with numeric scalar items, but the type of "Foo.embedding" is "String".',
        );
    });

    it('rejects vector index with non-alphanumeric name', () => {
        assertValidatorRejects(
            `
            type Foo @rootEntity {
                embedding: [Float!]! @vectorIndex(name: "bad-name", metric: COSINE, dimension: 128, nLists: 10)
            }
        `,
    ...
    it('rejects vector index with invalid storedValues path', () => {
        assertValidatorRejects(
            `
            type Foo @rootEntity {
                embedding: [Float!]! @vectorIndex(metric: COSINE, dimension: 128, nLists: 10, storedValues: ["tenant-id"])
            }
        `,
            'Type "Foo" does not have a field "tenant-id".',
        );
    });
