...
function selectModulesInGraphQLSource({
    ...
    for (const typeDef of parsedSource.document.definitions) {
        if (isTypeDefinitionNode(typeDef)) {
            ...
            if (!type.effectiveModuleSpecification.includedIn(selectedModules)) {
                ...
                // remove whole type
                changes.push({ oldLocation: typeDef.loc });
                continue;
            }
            ...
            if (typeDef.kind === Kind.OBJECT_TYPE_DEFINITION) {
                ...
                for (const fieldDef of typeDef.fields) {
                    ...
                    if (!field.effectiveModuleSpecification.includedIn(selectedModules)) {
                        ...
                        // remove field
                        changes.push({ oldLocation: fieldDef.loc });
                        break;
                    }
    ...
    for (let i = 0; i <= changes.length; i++) {
        ...
        const endOfSnippetToInclude = change ? change.oldLocation.start : source.body.length;
        if (endOfSnippetToInclude < currentPosition) {
            throw new Error(
                `Changes are not in order: found ${endOfSnippetToInclude} after ${currentPosition}`,
            );
        }
        output += source.body.substring(currentPosition, endOfSnippetToInclude);
        if (change && change.replacement !== undefined) {
            output += change.replacement;
        }
        if (change) {
            currentPosition = changes[i].oldLocation.end;
        }
    }
...
function changeModuleDirectiveIfNecessary(
    ...
    if (removeModuleDeclarations) {
        ...
        return { oldLocation: modulesDirectve.loc };
    }
    ...
    return { oldLocation: modulesDirectve.loc, replacement };
}
