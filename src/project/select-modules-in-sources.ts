...
function changeIndexIfNecessary(
    ...
    for (const field of fields) {
        // it's easier to reconstruct the IndexField instances than using type.indices because
        // we would need to find the matching IndexField instances
        ...
        if (fieldsInPath.some((f) => !f.effectiveModuleSpecification.includedIn(selectedModules))) {
            ...
            if (fieldsSoFar.length) {
                ...
                // (but first discuss whether the prefix-trimming behavior is actually what we want
                // - it can be confusing because it only applies to non-unique indices)
