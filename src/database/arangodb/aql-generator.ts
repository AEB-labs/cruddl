...
register(AggregationQueryNode, (node, context) => {
    ...
    switch (node.operator) {
        ...
        case AggregationOperator.DISTINCT:
            // TODO NXT-7991 move the distinct aggregation into TraversalQueryNode
            // if we do it here, the DISTINCT operates on the mapped result, which might be
            // identical for different nodes (if you don't selct the "id" field or similar)
...
register(TraversalQueryNode, (node, context) => {
    if (node.relationSegments.length && node.fieldSegments.length) {
        if (node.orderBy.isUnordered()) {
            return processTraversalWithRelationAndFieldSegmentsWithoutSort(node, context);
        } else {
            return processTraversalWithRelationAndFieldSegmentsWithSort(node, context);
        }
        ...
        return processTraversalWithOnlyFieldSegments(node, context);
    } else {
...
function processTraversalWithRelationAndFieldSegmentsWithoutSort(
    node: TraversalQueryNode,
    context: QueryContext,
) {
    ...
    // if there is a list field segment, the mapFrag will produce a list,
    // so a simple RETURN mapFrag would result in nested lists
    ...
    if (node.fieldSegments.some((f) => f.isListSegment)) {
        ...
        const limitClause = generateLimitClause(limitArgs);
        if (node.resultIsList) {
            return aqlExt.parenthesizeList(
                forStatementsFrag,
                limitClause ?? aql``,
                aql`RETURN ${outerMapFrag}`,
            );
        } else {
            if (limitClause) {
                throw new Error(`Cannot have limit on non-list traversal result`);
            }
            return aqlExt.parenthesizeObject(forStatementsFrag, aql`RETURN ${outerMapFrag}`);
        }
    }
}

/*
  TODO NXT-7991 decide whether we need this. if we do, explain it
  ...
  we could also just produce this:
  ...
  the difference would be that the CalculationNode that's fed into the inner FOR would be bigger
  (e.g. also include big fields on the items)
  for the reduce-extraction-to-projection optimization, this does not make a difference because it
  does not look inside arrays.
  however, I think it might still be beneficial the way we currently do it so the CalculationNode
  does not include potentially big side fields at all. We should check that.
 */
function processTraversalWithRelationAndFieldSegmentsWithSort(
    node: TraversalQueryNode,
    context: QueryContext,
) {
    if (!node.relationSegments.length || !node.fieldSegments) {
    ...
    // so a simple RETURN mapFrag would result in nested lists
    // -> we iterate over the items again to flatten the lists
    const forOrLetItemStatement = node.fieldSegments.some((f) => f.isListSegment)
        ? aql`FOR ${itemVar} IN ${outerMapFrag}`
        : aql`LET ${itemVar} = ${outerMapFrag}`;
...
function processTraversalWithOnlyFieldSegments(
    node: TraversalQueryNode,
    context: QueryContext,
): AQLFragment {
    ...
    // If we don't have a SORT clause, we can put everything into array expansions.
    ...
    if (node.orderBy.isUnordered()) {
        ...
        const innerContext = context.introduceVariableAlias(node.itemVariable, itemVar);
        const returnValueFrag = node.innerNode
            ? processNode(node.innerNode, innerContext)
            : itemVar;
...
function getRelationTraversalForStatements({
    ...
    // TODO NXT-7991: this is a bit awkward, maybe switch back to just returning the effective var
    ...
    for (const segment of segments) {
        ...
        // TODO NXT-7991 we should refactor those cases to embed the aggregation in the traversal
        if (needsNullableVar) {
    ...
    if (segments[segments.length - 1].isListSegment && !preserveNullValues) {
        // using $var._key != null instead of $var != null because the latter prevents ArangoDB
        // from applying the reduce-extraction-to-projection optimization
...
function getFieldTraversalFragment({
    ...
    // TODO NXT-7991 ask arangodb whether this is stable behavior
    // [**] does not treat NULL as an empty list though: ([NULL])[**] == [ NULL ]
    ...
    for (const segment of segments) {
        ...
        if (segment.isListSegment) {
            ...
            // always use * to convert NULLs to empty lists
            ...
            if (index === lastListSegmentIndex) {
                ...
                // TODO NXT-7991 LIMIT with remainingSegments only works if items are non-null
                // (otherwise, we would count null items as well)
                ...
                return aql`${frag}${operatorPrefixFrag}${filterExprFrag}${limitExprFrag}${returnExprFrag}]${suffixFrag}`;
            } else {
                frag = aql`${frag}${operatorPrefixFrag}]`;
            }

            hasSeenList = true;
        } else if (index === segments.length - 1) {
            if (filterFrag || limitFrag) {
                // this means we have a path without any list segment but still a filter or limit
                // this should not occur:
                // - if there are no relation traversals, the whole traversal does not result in a list,
                //   so filter/limit does not make sense
                // - if there are relation traversals, filter/limit should have been applied there already
                //     - filter can just basically use fragment here to access the base object for filtering/
                //     - limit can just be on the outer FOR
                // TODO NXT-7991 actually implement filter and limit on the relation traversal in this case
                throw new Error(
                    `not implemented - mapFrag, filterFrag or limitFrag but last segment is not a list`,
                );
            }

            // last segment, but we did not have any list segments
            // (otherwise, we would have returned in index === lastListSegmentIndex above)
            if (mapFrag) {
                // frag is just a sequential field access without lists, so we can just apply the mapFrag on it
                return mapFrag(frag);
            }
        }
