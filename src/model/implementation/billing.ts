...
export class BillingEntityType implements ModelComponent {
    ...
    validate(context: ValidationContext): void {
        if (!this.rootEntityType) {
            context.addMessage(ValidationMessage.error(`No rootEntity with the name "${this.input.typeName}" is defined.`, this.input.typeNameLoc));
            return;
        }
        if (this.input.keyFieldName && !this.rootEntityType.fields.find(value => value.name === this.input.keyFieldName)) {
            context.addMessage(ValidationMessage.error(`The field "${this.input.keyFieldName}" is not defined in the type "${this.input.typeName}".`, this.input.keyFieldNameLoc));
            return;
        }
