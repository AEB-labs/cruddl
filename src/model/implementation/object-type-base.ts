...
export abstract class ObjectTypeBase extends TypeBase {
    ...
    validate(context: ValidationContext) {
        ...
        if (!this.fields.filter((f) => !f.isSystemField).length) {
            context.addMessage(
                ValidationMessage.error(`Object type "${this.name}" does not declare any fields.`, this.nameASTNode)
            );
        }
        ...
        for (const fields of duplicateFields) {
            ...
            for (const field of fields) {
                ...
                if (isSystemFieldCollision) {
                    ...
                    context.addMessage(
                        ValidationMessage.error(`Duplicate field name: "${field.name}".`, field.astNode)
                    );
                }
            }
        }
