...
export class Field implements ModelComponent {
    ...
    private validateName(context: ValidationContext) {
        ...
        if (this.name.startsWith('_')) {
            context.addMessage(ValidationMessage.error(`Field names cannot start with an underscore.`, undefined, this.astNode));
            return;
        }
        ...
        if (this.name.includes('_')) {
            context.addMessage(ValidationMessage.warn(`Field names should not include underscores.`, undefined, this.astNode));
            return;
        }
    ...
    private validateDefaultValue(context: ValidationContext) {
        ...
        if (this.type.kind !== TypeKind.SCALAR && this.type.kind !== TypeKind.ENUM && this.type.kind !== TypeKind.VALUE_OBJECT) {
            context.addMessage(ValidationMessage.error(`Default values are only supported on fields of scalar, enum, and value object type.`, undefined, this.input.defaultValueASTNode || this.astNode));
            return;
        }
