        ...
        for (const types of duplicateTypes) {
            for (const type of types) {
                ...
                if (builtInTypeNames.has(type.name)) {
                    ...
                    context.addMessage(ValidationMessage.error(`Type name "${type.name}" is reserved by a built-in type.`, undefined, type.nameASTNode));
                } else {
                    context.addMessage(ValidationMessage.error(`Duplicate type name: "${type.name}".`, undefined, type.nameASTNode));
                }
            }
        }
    }
