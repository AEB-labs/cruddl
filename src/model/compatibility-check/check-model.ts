...
import { getYamlNodePairAtPath, patchBeforeCommentFromParentMap, safeParseDocument } from './utils';
...
export function generateYamlAddInMapChangesForTypeI18n(args: {
    ...
    const baselineTypeConfigs = baselineType.model.i18n.getTypeLocalizationConfigs(
        baselineType.name,
    );
    const toCheckTypeConfigs = toCheckModelI18n.getTypeLocalizationConfigs(baselineType.name);
    ...
    if (
        baselineType.kind === TypeKind.ROOT_ENTITY ||
        baselineType.kind === TypeKind.CHILD_ENTITY ||
        baselineType.kind === TypeKind.VALUE_OBJECT ||
        baselineType.kind === TypeKind.ENTITY_EXTENSION
    ) {
    ...
    if (baselineType.kind === TypeKind.ENUM) {
        result.push(
