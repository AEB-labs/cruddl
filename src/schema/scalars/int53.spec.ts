...
import { GraphQLInt53 } from './int53.js';

describe('Int53', () => {
    describe('parseLiteral', () => {
        it('accepts positive ints', () => {
            expect(GraphQLInt53.parseLiteral({ kind: Kind.INT, value: '123' }, {})).to.equal(123);
        });

        it('accepts negative ints', () => {
            expect(
                GraphQLInt53.parseLiteral({ kind: Kind.INT, value: '-9007199254740991' }, {}),
            ).to.equal(-9007199254740991);
        });

        it('rejects floats', () => {
            expect(() =>
                GraphQLInt53.parseLiteral({ kind: Kind.FLOAT, value: '123.4' }, {}),
            ).to.throw('Int53 cannot represent non-integer value: 123.4');
        });

        it('rejects numeric strings', () => {
            expect(() =>
                GraphQLInt53.parseLiteral({ kind: Kind.STRING, value: '123' }, {}),
            ).to.throw('Int53 cannot represent non-integer value: "123"');
        });

        it('rejects ints that are too large', () => {
            expect(() =>
                GraphQLInt53.parseLiteral({ kind: Kind.INT, value: '9007199254740992' }, {}),
            ).to.throw(
