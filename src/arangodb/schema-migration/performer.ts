...
export class MigrationPerformer {
    ...
    async waitForVectorIndexReady(
        ...
        while (true) {
            ...
            if (!index) {
                // With inBackground: true, the index may not appear in the catalog
                // immediately after ensureIndex returns. Treat "not found yet" as a
                // transient state and keep retrying until the timeout is reached.
                if (Date.now() - startTime > timeoutMs) {
                    throw new Error(
                        `Vector index "${indexName}" not found on collection "${collectionName}" within ${timeoutMs}ms`,
                    );
                }
