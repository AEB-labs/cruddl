...
function main() {
    ...
    const generatedCode = generateValidatorCode(schema);
    const rewrittenCode = rewriteRequireStatementsToImports(generatedCode);
    ...
    fs.writeFileSync(validatorCjsAbsPath, generatedCode);
    console.log(`Wrote ${validatorCjsAbsPath}`);

    fs.writeFileSync(validatorAbsPath, rewrittenCode);
    console.log(`Wrote ${validatorAbsPath}`);

    const exportName = extractDefaultExportName(rewrittenCode);
